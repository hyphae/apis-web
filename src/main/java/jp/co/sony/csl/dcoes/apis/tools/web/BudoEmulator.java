 ...
 * Verticle.
 * Emulates providing {@code BUDO} information to main_controller in the DCDC
 * emulator environment.
 * Provides the following API.
 * - /shutdown : Shuts down unit specified by {@code unitId} or all units
 * (including apis-tools participating in clusters).
 * - /setOperationMode : Changes the local Power Sharing mode or global Power
 * Sharing mode of the unit specified by {@code unitId} to {@code value}
 * - /deals : Gets Power Sharing information
 * - /unitIds : Gets list of IDs of units set by POLICY
 * - /getStatus : Gets whether of not the global Power Sharing mode is a mode
 * that allows Power Sharing
 ...
 *         以下の API を提供する.
 *         - /shutdown : {@code unitId} で指定したユニットまたは全ユニット ( クラスタに参加する apis-tools
 *         含む ) をシャットダウンする
 *         - /setOperationMode : {@code unitId}
 *         で指定したユニットのローカル融通モードまたはグローバル融通モードを {@code value} に変更する
 *         - /deals : 融通情報を取得する
...
public class BudoEmulator extends AbstractVerticle {
		...
		vertx.createHttpServer().requestHandler(req -> {
			...
			try {
				if (req.path().equals("/shutdown")) {
					...
					if (unitId != null) {
						vertx.eventBus().request(ServiceAddress.shutdown(unitId), null).onComplete(rep -> {
							if (rep.succeeded()) {
								...
								JsonObject result = new JsonObject().put("succeeded", Boolean.FALSE).put("error",
										String.valueOf(rep.cause()));
								req.response().setChunked(true).putHeader("content-type", "application/json")
										.end(result.encode() + '\n');
							}
						});
					...
					String address = (unitId != null) ? ServiceAddress.User.operationMode(unitId)
							: ServiceAddress.operationMode();
					if (log.isInfoEnabled())
						log.info("setting " + ((unitId != null) ? "unit " + unitId + "'s" : "global")
								+ " operationMode : " + value + " ...");
					DeliveryOptions options = new DeliveryOptions().addHeader("command", "set");
					vertx.eventBus().request(address, value, options).onComplete(rep -> {
						if (rep.succeeded()) {
							JsonObject result = new JsonObject().put("succeeded", Boolean.TRUE);
							req.response().setChunked(true).putHeader("content-type", "application/json")
									.end(result.encode() + '\n');
					...
					vertx.eventBus().<JsonArray>request(ServiceAddress.Mediator.deals(), null).onComplete(rep -> {
						if (rep.succeeded()) {
					...
					vertx.eventBus().<JsonArray>request(ServiceAddress.GridMaster.unitIds(), null).onComplete(rep -> {
						if (rep.succeeded()) {
			...
			if (res.succeeded()) {
				if (log.isInfoEnabled())
					log.info("budo emulation http service started on port : " + port);
	...
	/**
	 * Sets the global Power Sharing mode, gets the global Power Sharing mode again,
	...
	private void setGlobalOperationModeAndReturn_(HttpServerRequest req, String globalOperationMode) {
		if (log.isInfoEnabled())
			log.info("setting global operationMode : '" + globalOperationMode + "' ...");
		DeliveryOptions options = new DeliveryOptions().addHeader("command", "set");
		// Sets Global Power Sharing mode
		// グローバル融通モードをセットする
		vertx.eventBus().request(ServiceAddress.operationMode(), globalOperationMode, options).onComplete(rep -> {
			if (rep.succeeded()) {
				...
				log.error("Communication failed on EventBus ; ", rep.cause());
				JsonObject result = new JsonObject().put("active", Boolean.FALSE).put("error",
						String.valueOf(rep.cause()));
				req.response().setChunked(true).putHeader("content-type", "application/json")
						.end(result.encode() + '\n');
			}
	 ...
	 * Returns {@link Boolean#TRUE} if {@code autonomous} or {@code heteronomous},
	 * otherwise returns {@link Boolean#FALSE}.
	 * 
	 * @param completionHandler The completion handler
	 *                          グローバル融通モードを取得する.
	 *                          {@code autonomous} または {@code heteronomous} なら
	 *                          {@link Boolean#TRUE}, それ以外なら {@link Boolean#FALSE}
	 *                          を返す.
	 * @param completionHandler the completion handler
	 */
	private void checkGlobalOperationMode_(Handler<AsyncResult<Boolean>> completionHandler) {
		...
		vertx.eventBus().<String>request(ServiceAddress.operationMode(), null, options).onComplete(repOperationMode -> {
	 ...
	 * @return Power Sharing information {@link JsonObject} belonging to ancient
	 *         BUDO system era
	...
	private JsonObject convertDealInfo_(JsonObject deal) {
		...
		if (chargingUnit != null)
			result.put("chargingUnit", chargingUnit);
		if (dischargingUnit != null)
			result.put("dischargingUnit", dischargingUnit);
		if (isMasterDeal != null)
			result.put("isMasterDeal", isMasterDeal);
		if (request != null)
			result.put("request", request.toUpperCase());
		if (requester != null)
			result.put("requester", requester);
		if (responder != null)
			result.put("responder", responder);
		if (startTime != null)
